...
import klisp.FixedPoint
import klisp.IFixedPoint
import klisp.None
import klisp.Option
import klisp.Some
...
abstract class Pattern {
    ...
    abstract class Attribute<A>(private val bottom: A,
        ...
        val value: A
            get() {
                ...
                // (3) It needs to be computed (generation < FIXED_POINT.generation).
                if (fixed || attributes.stabilized || (attributes.generation == FIXED_POINT.generation()))
                    return currentValue
                else {
    ...
    protected val attributes = object : Attributes {
        ...
        override fun fix() {
            this.generation = FIXED_POINT.generation()
            if (FIXED_POINT.master() === null) {
                FIXED_POINT.master(this)
                do {
                    FIXED_POINT.incGeneration()
                    FIXED_POINT.changed(false)
                    updateAttributes()
                } while (FIXED_POINT.changed())
                FIXED_POINT.stabilized(true)
                FIXED_POINT.incGeneration()
                updateAttributes()
                FIXED_POINT.reset()
            } else {
...
/**
 * FIXED_POINT tracks the state of a fixed point algorithm for the attributes of a grammar.
 *
 * In case there are fixed points running in multiple threads, each attribute is thread-local.
 */
private val FIXED_POINT: IFixedPoint = FixedPoint()
...
class ParsingMachine<T : Token>(private val lang: Pattern, private val input: LiveStream<T>) {
    ...
    private fun search() {
        while (highTodo.isNotEmpty() || lowTodo.isNotEmpty()) {
            ...
            highTodo += (newConfs.filter { it.lang.firstc.isNotEmpty() })
            lowTodo += (newConfs.filter { it.hasMarks })
            println("l: ${lowTodo.size}, h: ${highTodo.size}")
...
data class TokenParser(val tag: String) : Parser<Token>() {
    override val compile by lazy { TokenPattern(tag, false) }
}
...
object EpsParser : Parser<Unit>() {
    override val compile by lazy { TokenPattern(HardEps) }
}
...
object EmptyParser : Parser<Nothing>() {
    override val compile by lazy { EmptyPattern }
}

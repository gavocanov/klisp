...
object TerminalCapabilities {
    ...
    fun detectANSISupport(default: TermColors.Level = NONE): TermColors.Level {
        ...
        when (Platform.getenv("COLORTERM")?.toLowerCase()) { // gist.github.com/XVilka/8346728
            "24bit", "truecolor" -> return TRUECOLOR
        }

        when (Platform.getenv("TERM_PROGRAM")?.toLowerCase()) {
            "vscode" -> ANSI256 // github.com/Microsoft/vscode/pull/30346
            "hyper" -> TRUECOLOR // stackoverflow.com/q/7052683
            "apple_terminal" -> ANSI256
            "iterm.app" -> {
                val ver = Platform.getenv("TERM_PROGRAM_VERSION")?.toIntOrNull()
                return if (ver != null && ver >= 3) TRUECOLOR else ANSI256
            }
        }

        val term = Platform.getenv("TERM")?.toLowerCase()
        ...
        return when (term) {
            "cygwin" -> when {
                ...
                Platform.getProperty("os.name") == "Windows 10" -> TRUECOLOR
                else -> ANSI256
            }
    ...
    fun consoleAvailable(): Boolean = Platform.console()
}

...
open class AnsiCode(protected val codes: List<Pair<List<Int>, Int>>) : (String) -> String {
    ...
    override fun invoke(p1: String) = if (p1.isEmpty()) "" else open + nest(p1) + close
    ...
    private fun nest(text: String) = ansiCloseRe.replace(text) {
        ...
        val codes = it.groupValues[1].splitToSequence(';').flatMap { s ->
            s.toInt().let { n ->
                if (atEnd && n in openCodesByCloseCode) emptySequence()
                else (openCodesByCloseCode[n]?.asSequence() ?: sequenceOf(n))
            }
        }
